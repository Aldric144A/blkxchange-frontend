...
export const BannerAd: React.FC<BannerAdProps> = ({ page }) => {
  const [ad, setAd] = useState<Ad | null>(null);

  useEffect(() => {
    const fetchAd = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/ads/${page}?placement=top`);
  ...
  return (
    <div className="w-full bg-[#1A1A1A] border-b border-[#C5A14E] py-2">
